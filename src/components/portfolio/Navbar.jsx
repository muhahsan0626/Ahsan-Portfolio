    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
        ...
        >
          ...
          <span className="font-mono">protfolio<span className="text-primary animate-pulse">_</span></span>
        </button>
        ...
        <div className="hidden lg:flex items-center gap-1">
          {navItems.map(item => (
            ...
            >
              <span className={`mr-1 ${activeSection === item.id ? 'text-primary' : 'text-transparent'}`}>{'>'}</span>
              {item.label}
            </button>
          ))}
          <Button
        ...
        <div className="lg:hidden bg-card/95 backdrop-blur-xl border-t border-primary/20 shadow-lg">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 space-y-1">
            {navItems.map(item => (
              ...
              >
                <span className={`mr-2 ${activeSection === item.id ? 'text-primary' : 'text-transparent'}`}>{'>'}</span>
                {item.label}
              </button>
            ))}
            <div className="pt-2">
